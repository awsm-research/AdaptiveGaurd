...
def apply_lora(model, args):
    """Apply a simplified LoRA-like approach by freezing most parameters 
    and only fine-tuning selected layers for classification performance"""
    # Freeze all parameters first
    for param in model.parameters():
        param.requires_grad = False
        
    # Unfreeze selected layers for classification-focused fine-tuning
    trainable_params = 0
    total_params = 0
    ...
    for name, param in model.named_parameters():
        ...
        # Target these specific components:
        # 1. The classification layer (score) - critical for classification
        # 2. Last 3 transformer blocks for GPT2 to capture high-level features
        # 3. Selectively unfreeze key attention components for better adaptation
        if 'score' in name or any(f'h.{i}' in name for i in [9, 10, 11]):
            # For transformer blocks, focus on attention mechanisms and fully connected layers
            if ('score' in name or 
                'attn' in name or  # attention mechanisms
                'mlp' in name):    # MLP layers for processing
...
def main():
    ...
    parser.add_argument("--lora_target_modules", default="q_proj,v_proj,k_proj,o_proj,gate_proj,down_proj,up_proj",
    ...
    if args.do_retrain:
        ...
        if args.continual_learning_examples > 0:
            # Re-train on new data using LoRA
            train_dataset = TextDataset(tokenizer, args, file_type='train')
            
            # Fine-tune the model on the training data with LoRA
        ...
        # Always do evaluation for continual learning
        ...
        if args.eval_data_file:
            validation_dataset = TextDataset(tokenizer, args, file_type='eval')
        ...
        # Combine results
        all_results = {
            "attack_type": os.path.basename(os.path.dirname(args.test_data_file)) if args.test_data_file else os.path.basename(os.path.dirname(args.train_data_file)),
        ...
        # Return as we've completed the continual learning
        return
...
def continual_learning_train(args, model, tokenizer, train_dataset):
    """Train the model on a single batch for continual learning with LoRA"""
    # build dataloader
    train_sampler = RandomSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=0)
    ...
    # Apply LoRA to the model if LoRA parameters are provided
    if hasattr(args, 'lora_r') and args.lora_r > 0:
    ...
    # Train!
    lora_status = "with LoRA" if hasattr(args, 'lora_r') and args.lora_r > 0 else ""
    logger.info(f"***** Running continual learning training {lora_status} *****")
    logger.info("  Num examples = %d", len(train_dataset))
    logger.info("  Num Epochs = %d", args.epochs)
    logger.info("  Instantaneous batch size per GPU = %d", args.train_batch_size//max(args.n_gpu, 1))
